...
public class DetailSedudo extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Log.d("DetailSedudo", "Terima id_wisata: " + idWisata + ", extra nama=" + namaExtra);
        ...
        call.enqueue(new Callback<WisataModel>() {
            ...
            public void onResponse(Call<WisataModel> call, Response<WisataModel> response) {
                if (response.isSuccessful() && response.body() != null) {
                    ...
                    // ✅ Ambil gambar berdasarkan ID wisata
                    int imageResId = getDrawableForWisata(idWisata);
                    imgHeader.setImageResource(imageResId);

                    Log.d("DetailSedudo", "HargaDewasa=" + hargaDewasa + ", HargaAnak=" + hargaAnak);

                } else {
                    Toast.makeText(DetailSedudo.this, "Data tidak ditemukan", Toast.LENGTH_SHORT).show();
                }
            }

            @Override
            public void onFailure(Call<WisataModel> call, Throwable t) {
                Toast.makeText(DetailSedudo.this, "Gagal ambil data: " + t.getMessage(), Toast.LENGTH_SHORT).show();
            }
        });
